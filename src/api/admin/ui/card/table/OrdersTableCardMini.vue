...
<template>
  <div @click="order.id === activeDish ? (activeDish = null) : (activeDish = order.id)"
    ...
    <div class="flex gap-x-2 flex-col items-center text-center">
      ...
      <div class="flex px-5 py-3 justify-self-center w-max ">
        <div @click.stop="isActiveStatusMenu === order.id ? isActiveStatusMenu = false : isActiveStatusMenu = order.id"
          :class="styleStatusOrder"
          class="relative text-white cursor-pointer px-2 py-0.5 rounded-xl max-sm:px-1.5 max-sm:text-[12px]">
          {{ order.current_status }}
          <div @click="sendMessage(order.id)" v-show="isActiveStatusMenu === order.id"
