...
<template>
  ...
  <div class="mx-7 px-20 w-full p-5 rounded-2xl max-2xl:px-0 max-md:mx-0">
      ...
      <form ref="formDishEdit" class="bg-[#111827] rounded-xl p-5 max-w-full">
        <div
        ...
        <div class="grid grid-cols-3 w-full justify-between gap-x-10 max-lg:gap-x-2">
          ...
          <div class="p-2 rounded-lg max-md:col-span-full">
            ...
            class="block w-full border-gray-600 border-[1px] bg-gray-800 rounded-lg px-3 py-2 focus:outline-0 focus-within:border focus-within:border-gray-600"
            >
              <option v-for="category in adminMenuStore.adminMenu" :key="category.id" :value="category.id">{{ category.name }}</option>
            </select>
          </div>
          <div class="p-2 rounded-lg max-md:col-span-full">
          ...
          <template v-for="(characteristic, key) in adminMenuStore.adminEditDish?.characteristics" :key="key">
            <div
            @click="
              ...
              >
              <span>
                {{ key + 1 }} характеристика
              </span>
              <div v-show="key !== 0" @click.stop="adminMenuStore.delateCharacteristics(key)"><IconBasket/></div>
            </div>
          ...
          <div class="col-span-full p-2 max-md:px-0 flex flex-wrap gap-y-2">
            <div v-for="(ingredient, index) in adminMenuStore.adminEditDish.composition" :key="index" class="px-2 py-1 flex items-center">
              <input
