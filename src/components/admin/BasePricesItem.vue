...
<template>
  <div>
    <h3
      class="w-full px-5 py-3 bg-gray-800 hover:bg-gray-700 border-white border-y duration-200"
      @click="isOpen === cityIndex ? (isOpen = null) : (isOpen = cityIndex)"
    >
      {{ adminMenuStore.adminPrices[cityIndex].city }}
    </h3>
    <div class="">
      <div class="grid grid-cols-3" v-show="cityIndex === isOpen">
        <PricesSetting
          v-for="(price, index) in adminMenuStore.adminPrices[cityIndex]?.prices"
          :indexCity="cityIndex"
          :key="price"
          :indexSetting="index"
        />
        <div
          @click="adminMenuStore.addPrice(cityIndex)"
